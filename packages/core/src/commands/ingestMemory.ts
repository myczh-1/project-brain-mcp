...
const MEMORY_HANDLERS: Record<MemoryType, MemoryHandler> = {
  project_spec: {
    ...
    async execute({ cwd, payload, source, type, storage }) {
  ...
  change_spec: {
    ...
    async execute({ cwd, payload, type, storage }) {
      const changePayload = payload as ChangeSpecPayload;
  ...
  note: {
    validate(payload) {
      ...
      return asTrimmedString((payload as NotePayload).note) ? null : 'note.payload.note is required.';
    },
    warnings(payload) {
