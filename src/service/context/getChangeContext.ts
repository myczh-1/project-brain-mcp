...
import { estimateAllMilestones, progressSummaryToOverallEstimation, estimateProgressSummary } from '../../core/understanding/estimateProgress.js';
...
function buildRisks(
  ...
  if (!projectSpec) {
    risks.push('No project-spec found; stable governance rules are missing from context.');
  }
  if (change.acceptance_criteria.length === 0) {
    risks.push('Change has no acceptance criteria yet.');
  }
  if (change.affected_areas.length === 0) {
    risks.push('Change does not declare affected areas, so code navigation may be noisy.');
  }
  if (decisionsCount === 0) {
    risks.push('No related decisions found; rationale history may be incomplete.');
  }
  if (progressCount === 0) {
    risks.push('No change-linked progress entries found.');
  }
  if (commitsCount === 0) {
    risks.push('No recent git evidence found.');
  }
...
export async function changeContext(input: ChangeContextInput): Promise<ChangeContextOutput> {
  ...
  const milestoneEstimations = milestones.length > 0 ? estimateAllMilestones(milestones, commits, hotPaths) : [];
  const overallProgress = progressSummaryToOverallEstimation(
    estimateProgressSummary(milestones, milestoneEstimations, commits, hotPaths)
  );
  const recommendations = recommendNextActions(milestones, commits, hotPaths, progress, decisions, [overallProgress, ...milestoneEstimations]);
